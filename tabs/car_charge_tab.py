...
def render(df, from_date, to_date, _filter_df_by_view_range, udeladning_pris):
    ...
    st.info("Se hvad du betaler for strøm til bilen og hvor meget Clever refunderer. Udfyld gerne dit forbrug fra Clever-appen for mere præcise tal.")
    ...
    try:
        ...
        med_clever = None
        uden_clever = None
        if 'total_udgift_ved_clever_abbonemnt' in locals() or 'display_table' in locals():
            # Prøv at bruge display_table hvis det findes
            if 'display_table' in locals():
                med_clever = display_table['total_udgift_ved_clever_abbonemnt'].sum() if 'total_udgift_ved_clever_abbonemnt' in display_table.columns else None
                uden_clever = display_table['total_udgift_uden_clever_abbonemnt'].sum() if 'total_udgift_uden_clever_abbonemnt' in display_table.columns else None
        # Alternativt prøv monthly_car hvis display_table ikke findes
        if med_clever is None or uden_clever is None:
            if 'monthly_car' in locals():
