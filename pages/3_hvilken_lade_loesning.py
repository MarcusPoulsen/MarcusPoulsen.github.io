...
cols = st.columns(3)
km_per_year = cols[0].number_input("Km om året", min_value=0, value=20000, step=100)
consumption_per_100km = cols[1].number_input("Elbil forbrug (kWh/100 km)", min_value=0.0, value=20.0, step=0.1)
ladetab_pct = cols[2].number_input("Antaget ladetab (%)", min_value=0.0, max_value=100.0, value=8.0, step=0.1)

cols2 = st.columns(3)
hjemme_pct = cols2[0].number_input("Hjemmeopladning procent (%)", min_value=0.0, max_value=100.0, value=90.0, step=0.1)
saeson_effekt = cols2[1].radio("Sæson effekt medregnes?", ["ja", "nej"], index=0)
hjemme_pris_kwh = cols2[2].number_input("Hjemmeladning pris pr kWh (DKK)", min_value=0.0, value=1.0, step=0.01)
